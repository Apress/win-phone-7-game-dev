...
namespace MultipleGameObjects
{
    internal class BallObject : SpriteObject
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (PositionX > _game.Window.ClientBounds.Width - OriginX)
            {
                // Reset back to the right edge
                PositionX = _game.Window.ClientBounds.Width - OriginX;
                // Reverse the x velocity
                _xadd = -_xadd;
                // Add to the wobble
                _wobble += Math.Abs(_xadd);
            ...
            if (PositionY >= _game.Window.ClientBounds.Bottom - OriginY)
            {
                // Reset back to the bottom of the window
                PositionY = _game.Window.ClientBounds.Bottom - OriginY;
