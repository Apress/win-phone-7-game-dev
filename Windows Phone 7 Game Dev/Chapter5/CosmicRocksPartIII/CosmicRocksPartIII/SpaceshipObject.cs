...
namespace CosmicRocksPartIII
{
    internal class SpaceshipObject : GameFramework.SpriteObject
    {
        ...
        private CosmicRocksPartIIIGame _game;
        ...
        // The number of bullets the player max have active at once
        private const int MaxBullets = 4;

        // The thruster sound effect instance
        private SoundEffectInstance _thrusterSound;

        //-------------------------------------------------------------------------------------
        // Class constructors

        internal SpaceshipObject(CosmicRocksPartIIIGame game, Texture2D texture, Vector2 position)
            : base(game, position, texture)
        {
            ...
            // Create and initialise the thruster sound effect instance
            _thrusterSound = _game.SoundEffects["Thruster"].CreateInstance();
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (_hyperspaceZoomAdd != 0)
            {
                ...
                if (_hyperspaceZoom >= 110)
                {
                    // Yes, so move to the new location
                    // Start to zoom back out
                    _hyperspaceZoomAdd = -_hyperspaceZoomAdd;
                    // Set a random new position
                    PositionX = GameHelper.RandomNext(0, _game.Window.ClientBounds.Width - SpriteTexture.Width) + SpriteTexture.Width / 2;
                    PositionY = GameHelper.RandomNext(0, _game.Window.ClientBounds.Height - SpriteTexture.Height) + SpriteTexture.Height / 2;
                    // Play sound effect
                    _game.SoundEffects["HyperspaceIn"].Play();
                }
            ...
            // Did the player press 'H' on the keyboard?
            // (Allows us to hyperspace on the emulator with no multitouch)
            if (Keyboard.GetState().IsKeyDown(Keys.H)) Hyperspace();
        ...
        private void RotateToFacePoint(Vector2 point)
        {
            ...
            // XNA puts 0 degrees upwards, whereas Atan2 returns it facing left, so add 90 degrees
            // to rotate the Atan2 value into alignment with XNA
        ...
        private void FireBullet()
        {
            ...
            // Play a sound
            _game.SoundEffects["BulletSound"].Play(0.75f, 0.2f, 0.0f);
        }
        ...
        private void Hyperspace()
        {
            ...
            // Play sound effect
            _game.SoundEffects["HyperspaceOut"].Play();
            // Stop other sounds
            StopThrusterSound();
        }
        ...
        private void Explode()
        {
            ...
            // Play a sound effect
            _game.SoundEffects["SpaceshipExplosion"].Play();
            _game.SoundEffects["SpaceshipExplosion"].Play(1.0f, GameHelper.RandomNext(-0.1f, 0.1f), 0.0f);
            // Stop other sounds
            StopThrusterSound();
        }
        ...
        {
            if (_thrusterSound.State != SoundState.Playing) _thrusterSound.Play();
        }
        ...
        {
            if (_thrusterSound.State != SoundState.Stopped) _thrusterSound.Stop();
        }
