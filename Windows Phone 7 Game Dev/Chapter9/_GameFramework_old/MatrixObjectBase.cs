...
{
    [DataContract]
    public class MatrixObjectBase : GameObjectBase
    {

        //-------------------------------------------------------------------------------------
        // Class constructors

        public MatrixObjectBase()
            : base()
        {
        }
        ...
        public virtual Texture2D ObjectTexture
        {
            ...
            set
            {
                if (_objectTexture != value)
                {
                    // Set the Object texture
                    _objectTexture = value;
                    // Set the texture name
                    _objectTextureName = Game.GetContentObjectName<Texture2D>(Game.Textures, value);
                }
            }
        }

        /// <summary>
        /// The name of the texture being used by this Object
        /// </summary>
        private string _objectTextureName;
        /// <summary>
        /// Sets or returns the Object texture name. This can be serialized when the game is tombstoned.
        /// </summary>
        [DataMember]
        public virtual string ObjectTextureName
        {
            get
            {
            ...
            set
            {
                ...
                if (_objectTextureName != value)
                {
                    // Store the new name
                    _objectTextureName = value;
                    // Clear the stored texture so that the name is processed the name
                    // time the ObjectTexture property is called
                    _objectTexture = null;
                }
            }
        }
        ...
        /// The underlying texture being used by this Object
        /// </summary>
        private Texture2D _objectTexture2;
        /// <summary>
        /// A reference to the default texture used by this Object
        /// </summary>
        public virtual Texture2D ObjectTexture2
        {
            get
            {
                // Do we have a texture name but no actual texture?
                // This will be the case after recovering from being tombstoned.
                if (_objectTexture2 == null && !String.IsNullOrEmpty(_objectTexture2Name) && Game != null)
                {
                    // Get the texture back from the Textures collection
                    _objectTexture2 = Game.Textures[_objectTexture2Name];
                }
                // Return the Object texture
                return _objectTexture2;
            }
            set
            {
                if (_objectTexture2 != value)
                {
                    // Set the Object texture
                    _objectTexture2 = value;
                    // Set the texture name
                    _objectTexture2Name = Game.GetContentObjectName<Texture2D>(Game.Textures, value);
                }
            }
        }
        ...
        /// The name of the texture being used by this Object
        /// </summary>
        private string _objectTexture2Name;
        /// <summary>
        /// Sets or returns the Object texture name. This can be serialized when the game is tombstoned.
        /// </summary>
        [DataMember]
        public virtual string ObjectTexture2Name
        {
            get
            {
                // Return the texture name
                return _objectTexture2Name;
            }
            set
            {
                // Is this texture name different to the one we are already using?
                if (_objectTexture2Name != value)
                {
                    // Store the new name
                    _objectTexture2Name = value;
                    // Clear the stored texture so that the name is processed the name
                    // time the ObjectTexture2 property is called
                    _objectTexture2 = null;
                }
            }
        }
