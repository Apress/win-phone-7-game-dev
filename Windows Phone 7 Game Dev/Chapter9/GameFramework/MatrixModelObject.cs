...
namespace GameFramework
{
    ...
    public class MatrixModelObject : MatrixObjectBase
    {
        ...
        /// <summary>
        /// The underlying Model being used by this sprite
        /// </summary>
        private Model _objectModel;
        /// <summary>
        /// A reference to the default Model used by this sprite
        /// </summary>
        public Model ObjectModel
        {
            get
            {
                // Do we have a Model name but no actual Model?
                // This will be the case after recovering from being tombstoned.
                if (_objectModel == null && !String.IsNullOrEmpty(_modelName) && Game != null)
                {
                    // Get the Model back from the Models collection
                    _objectModel = Game.Models[_modelName];
                }
            ...
            set
            {
                if (_objectModel != value)
                {
                    // Set the sprite Model
                    _objectModel = value;
                    // Set the Model name
                    _modelName = Game.GetContentObjectName<Model>(Game.Models, value);
                }
            }
        }

        /// <summary>
        /// The name of the Model being used by this sprite
        /// </summary>
        private string _modelName;
        /// <summary>
        /// Sets or returns the sprite Model name. This can be serialized when the game is tombstoned.
        /// </summary>
        ...
        public virtual string ObjectModelName
        {
            ...
            set
            {
                // Is this Model name different to the one we are already using?
                if (_modelName != value)
                {
                    // Store the new name
                    _modelName = value;
                    // Clear the stored Model so that the name is processed the name
                    // time the SpriteModel property is called
                    _objectModel = null;
                }
            }
        }
